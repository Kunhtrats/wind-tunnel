...
import { Solver, SHAPES, equilibrium, speedToKmh, bodyAt } from './solver.mjs';
...
assert.ok(rest.u.every(u => Math.abs(u) < 1e-7), 'rest remains at rest');
assert.ok(rest.rho.every(r => Math.abs(r - 1) < 1e-6), 'rest preserves mass');
...
assert.throws(() => new Solver({ direction: 181 }));
assert.equal(new Solver({ width: 576 }).height, 288);
...
assert.throws(() => live.update({ ...liveConfig, speed: 1 }));
...
assert.equal(messages.at(-1).steps, workerSteps, 'worker live update preserves time');
